...
class ViT(nn.Module):
    ...
    def chop_to_patches(self, x, b, c):
        ...
        chopped_x = x.reshape(b, self.image_size // self.patch_size, self.image_size // self.patch_size, c, self.patch_size, self.patch_size)
    ...
    def chop_to_patches_with_conv(self, x, b, c):
        ...
        conv = nn.Conv2d(c, self.dim, kernel_size=self.patch_size, stride=self.patch_size, bias=False)
        chopped_x = conv(x)
        return chopped_x
